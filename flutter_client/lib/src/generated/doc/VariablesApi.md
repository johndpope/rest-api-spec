...
# **getLocalVariables**
> GetLocalVariables200Response getLocalVariables(fileKey)
...
final api = FigmaApi().getVariablesApi();
final String fileKey = fileKey_example; // String | File to get variables from. This can be a file key or branch key. Use `GET /v1/files/:key` with the `branch_data` query param to get the branch key.

try {
    final response = api.getLocalVariables(fileKey);
    print(response);
} catch on DioException (e) {
    print('Exception when calling VariablesApi->getLocalVariables: $e\n');
}
```
...
[**GetLocalVariables200Response**](GetLocalVariables200Response.md)
...
> GetPublishedVariables200Response getPublishedVariables(fileKey)
...
final api = FigmaApi().getVariablesApi();
final String fileKey = fileKey_example; // String | File to get variables from. This must be a main file key, not a branch key, as it is not possible to publish from branches.

try {
    final response = api.getPublishedVariables(fileKey);
    print(response);
} catch on DioException (e) {
    print('Exception when calling VariablesApi->getPublishedVariables: $e\n');
}
```
...
[**GetPublishedVariables200Response**](GetPublishedVariables200Response.md)
...
# **postVariables**
> PostVariables200Response postVariables(fileKey, postVariablesRequest)
...
final api = FigmaApi().getVariablesApi();
final String fileKey = fileKey_example; // String | File to modify variables in. This can be a file key or branch key. Use `GET /v1/files/:key` with the `branch_data` query param to get the branch key.
final PostVariablesRequest postVariablesRequest = ; // PostVariablesRequest | 

try {
    final response = api.postVariables(fileKey, postVariablesRequest);
    print(response);
} catch on DioException (e) {
    print('Exception when calling VariablesApi->postVariables: $e\n');
}
```
...
[**PostVariables200Response**](PostVariables200Response.md)
