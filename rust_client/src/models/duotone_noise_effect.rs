/*
 * Figma API
 *
 * This is the OpenAPI specification for the [Figma REST API](https://www.figma.com/developers/api).  Note: we are releasing the OpenAPI specification as a beta given the large surface area and complexity of the REST API. If you notice any inaccuracies with the specification, please [file an issue](https://github.com/figma/rest-api-spec/issues).
 *
 * The version of the OpenAPI document: 0.35.0
 * Contact: support@figma.com
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct DuotoneNoiseEffect {
    /// The string literal 'NOISE' representing the effect's type. Always check the type before reading other properties.
    #[serde(rename = "type")]
    pub r#type: Type,
    #[serde(rename = "color")]
    pub color: Box<models::Rgba>,
    /// Whether the noise effect is visible.
    #[serde(rename = "visible")]
    pub visible: bool,
    #[serde(rename = "blendMode")]
    pub blend_mode: models::BlendMode,
    /// The size of the noise effect
    #[serde(rename = "noiseSize")]
    pub noise_size: f64,
    /// The density of the noise effect
    #[serde(rename = "density")]
    pub density: f64,
    /// The string literal 'DUOTONE' representing the noise type.
    #[serde(rename = "noiseType")]
    pub noise_type: NoiseType,
    #[serde(rename = "secondaryColor")]
    pub secondary_color: Box<models::Rgba>,
}

impl DuotoneNoiseEffect {
    pub fn new(r#type: Type, color: models::Rgba, visible: bool, blend_mode: models::BlendMode, noise_size: f64, density: f64, noise_type: NoiseType, secondary_color: models::Rgba) -> DuotoneNoiseEffect {
        DuotoneNoiseEffect {
            r#type,
            color: Box::new(color),
            visible,
            blend_mode,
            noise_size,
            density,
            noise_type,
            secondary_color: Box::new(secondary_color),
        }
    }
}
/// The string literal 'NOISE' representing the effect's type. Always check the type before reading other properties.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum Type {
    #[serde(rename = "NOISE")]
    Noise,
}

impl Default for Type {
    fn default() -> Type {
        Self::Noise
    }
}
/// The string literal 'DUOTONE' representing the noise type.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum NoiseType {
    #[serde(rename = "DUOTONE")]
    Duotone,
}

impl Default for NoiseType {
    fn default() -> NoiseType {
        Self::Duotone
    }
}

